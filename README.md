...
Versione dati corrente: **v1.19.0** — 26 agosto 2026.
...
- 7 schede comunali;
- 11 aree tematiche;
- 149 indicatori nel catalogo canonico: 145 con valori incorporati e 4 climatici con storici separati;
...
- `indicatori/`: 145 pagine canoniche generate in build, una per indicatore con dati incorporati;
- `data/site-data.json`: catalogo canonico dei 149 indicatori, con dati incorporati per 145 e riferimenti ai file storici separati per i 4 climatici;
...
Il catalogo e i metadati dei 149 indicatori sono centralizzati in `data/site-data.json`. Gli storici climatici più pesanti restano nei file dedicati richiamati da `dataStorage`. Per aggiornamenti strutturali conviene modificare o rigenerare questi dati mantenendo per ogni indicatore:
...
- valida tutti i 149 indicatori canonici, la ripartizione fra 145 valori incorporati e 4 storici climatici separati e la coerenza della copertura dichiarata;
...
La build genera una pagina autonoma per ciascuno dei 145 indicatori incorporati, con URL canonica, confronto comunale in ordine alfabetico, serie storica quando disponibile, fonte, metodo, dati strutturati `Dataset` e breadcrumb. Le URL sono incluse nella sitemap con `lastmod`.
