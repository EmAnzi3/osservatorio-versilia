...
Versione dati corrente: **v1.18.0** — 25 agosto 2026.
...
- 7 schede comunali;
- 11 aree tematiche;
- 146 indicatori nel catalogo canonico: 142 con valori incorporati e 4 climatici con storici separati;
...
- `indicatori/`: 142 pagine canoniche generate in build, una per indicatore con dati incorporati;
- `data/site-data.json`: catalogo canonico dei 146 indicatori, con dati incorporati per 142 e riferimenti ai file storici separati per i 4 climatici;
...
Il catalogo e i metadati dei 146 indicatori sono centralizzati in `data/site-data.json`. Gli storici climatici più pesanti restano nei file dedicati richiamati da `dataStorage`. Per aggiornamenti strutturali conviene modificare o rigenerare questi dati mantenendo per ogni indicatore:
...
- valida tutti i 146 indicatori canonici, la ripartizione fra 142 valori incorporati e 4 storici climatici separati e la coerenza della copertura dichiarata;
...
La build genera una pagina autonoma per ciascuno dei 142 indicatori incorporati, con URL canonica, confronto comunale in ordine alfabetico, serie storica quando disponibile, fonte, metodo, dati strutturati `Dataset` e breadcrumb. Le URL sono incluse nella sitemap con `lastmod`.
