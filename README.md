...
Versione dati corrente: **v1.28.0** — 2 settembre 2026.
...
- 7 schede comunali;
- 11 aree tematiche;
- 180 indicatori nel catalogo canonico: 176 con valori incorporati e 4 climatici con storici separati;
...
- `indicatori/`: 176 pagine canoniche generate in build, una per indicatore con dati incorporati;
- `data/site-data.json`: catalogo canonico dei 180 indicatori, con dati incorporati per 176 e riferimenti ai file storici separati per i 4 climatici;
...
Il catalogo e i metadati dei 180 indicatori sono centralizzati in `data/site-data.json`. Gli storici climatici più pesanti restano nei file dedicati richiamati da `dataStorage`. Per aggiornamenti strutturali conviene modificare o rigenerare questi dati mantenendo per ogni indicatore:
...
- valida tutti i 180 indicatori canonici, la ripartizione fra 176 valori incorporati e 4 storici climatici separati e la coerenza della copertura dichiarata;
...
La build genera una pagina autonoma per ciascuno dei 176 indicatori incorporati, con URL canonica, confronto comunale in ordine alfabetico, serie storica quando disponibile, fonte, metodo, dati strutturati `Dataset` e breadcrumb. Le URL sono incluse nella sitemap con `lastmod`.
