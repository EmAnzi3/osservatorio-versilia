(() => {
  ...
  function renderMinMax() {
    ...
    const minTrend = linearTrend(series.years, series.tmin, 1995, 2015);
    const maxTrend = linearTrend(series.years, series.tmax, 1995, 2015);
    minmaxSummaryRoot.innerHTML = `
      <article><span>Minime · trend 1995–2015</span><strong>${signed(minTrend.delta, 2)} °C</strong><small>Media annua delle temperature minime giornaliere territoriali.</small></article>
      <article><span>Massime · trend 1995–2015</span><strong>${signed(maxTrend.delta, 2)} °C</strong><small>Media annua delle temperature massime giornaliere territoriali.</small></article>`;
    ...
    const xTicks = [1995, 2000, 2005, 2010, 2015].map(year => `<text class="climate-axis-label" x="${x(year)}" y="${height - 14}" text-anchor="middle">${year}</text>`).join('');
    minmaxChartRoot.innerHTML = `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Temperature minime e massime medie annue a ${escapeHtml(state.town)} dal 1995 al 2015">${grids}${xTicks}<path class="climate-min-line" d="${minMaxPath(years, series.tmin, x, y)}"></path><path class="climate-max-line" d="${minMaxPath(years, series.tmax, x, y)}"></path>${series.tmin.map((value, index) => `<circle class="climate-min-point" cx="${x(years[index])}" cy="${y(value)}" r="3"></circle>`).join('')}${series.tmax.map((value, index) => `<circle class="climate-max-point" cx="${x(years[index])}" cy="${y(value)}" r="3"></circle>`).join('')}</svg>`;
  }
