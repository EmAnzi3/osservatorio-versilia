a l’aggiornamento della classificazione ATECO.</div>`;
}
...
function renderAnalysis(){
  ...
  if(heading)heading.textContent=initialTownMeta?`${initialTownMeta.name} nel confronto territoriale`:(state.leftMode==='composition'?'Il nodo nei sette Comuni':'Il codice nei sette Comuni');
  ...
  if(!row){box.innerHTML=`<div class="selected-title"><code>${n.d?`${n.sec} · ${displayCode(n.d)}`:`Sezione ${n.sec}`}</code><h3>${escapeHtml(n.label||'Dicitura non disponibile')}</h3><p>${n.d?LEVELS[n.d.length]:'Sezione'}</p>${