#!/usr/bin/env python3
"""Finalizza e valida il contratto del catalogo pubblico v1.28.0.
...
VERSION = "v1.28.0"
UPDATED = "2 settembre 2026"
EXPECTED_TOWNS = 7
EXPECTED_THEMES = 11
EXPECTED_METRICS = 180
EXPECTED_INLINE = 176
EXPECTED_EXTERNAL = 4
...
def validate(site: dict, registry: dict) -> None:
    ...
    external = {key for key, metric in metrics.items() if metric.get("dataStorage", {}).get("type") == "external-climate"}
    ...
    for theme_key, theme in themes.items():
        ...
        section_metrics = [key for section in theme.get("sections", []) for key in section.get("metrics", [])]
    ...
    if missing or unknown or duplicated:
        raise RuntimeError(f"Riferimenti tematici incoerenti; orfani={missing}, sconosciuti={unknown}, duplicati={duplicated}")
    expected_registry = {"expectedMetricCount": EXPECTED_METRICS,"expectedInlineMetricCount": EXPECTED_INLINE,"expectedExternalMetricCount": EXPECTED_EXTERNAL}
...
def main() -> None:
    ...
    site = load(SITE_PATH); registry = load(REGISTRY_PATH)
    desired_site = copy.deepcopy(site); desired_registry = copy.deepcopy(registry)
    finalize(desired_site, desired_registry); validate(desired_site, desired_registry)
    if args.check:
        ...
        if desired_site != site: save(SITE_PATH, desired_site)
        if desired_registry != registry: save(REGISTRY_PATH, desired_registry)
    print(f"Catalogo {VERSION} verificato: {EXPECTED_METRICS} indicatori ({EXPECTED_INLINE} incorporati + {EXPECTED_EXTERNAL} esterni), {EXPECTED_THEMES} temi, {EXPECTED_TOWNS}/7 Comuni.")
