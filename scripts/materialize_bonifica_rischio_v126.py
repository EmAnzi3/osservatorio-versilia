...
def build_metrics(site,snap,gis):
    ...
    return {
      KEYS[0]:{'meta':meta(KEYS[0],'Lunghezza interventi PAB programmati','Interventi PAB · km','Somma delle lunghezze delle attività di manutenzione ordinaria programmate nel PAB 2026 nei sette Comuni. La misura è espressa in km-intervento: interventi ripetuti sullo stesso tratto vengono contati separatamente.','km','2026','Consorzio 1 Toscana Nord — Portale manutenzioni PAB 2026',['bonifica','pab','manutenzione','reticolo','corsi d’acqua','km-intervento'],'Il dato misura attività programmate e non chilometri fisici unici di reticolo.',base_snapshot),'sourceUrl':PORTAL_URL,'rows':rows(site,L,lambda v:it(v,2)+' km'),'aggregate':{'value':pub[KEYS[0]]['total'],'label':'Versilia · totale programmato','note':'Somma dei km-intervento programmati nei sette Comuni; non rappresenta la lunghezza fisica unica del reticolo sottoposto a manutenzione.'},'normalizedAggregate':None,'method':{'type':'Elaborazione da export ufficiale Consorzio 1 Toscana Nord','formula':'Somma del campo Metri per Comune / 1.000.','caveat':'Se lo stesso tratto è oggetto di più interventi nell’anno, la sua lunghezza ricorre più volte: il dato è km-intervento, non km di reticolo fisico.','coverage':'7/7','snapshot':base_snapshot}},
...
def main():
    ...
    additions=[]
    if "case 'km':" not in txt: additions.append("      case 'km': return `${number2.format(v)} km`;\n")
    if "case 'kmIntervention':" not in txt: additions.append("      case 'kmIntervention': return `${number2.format(v)} km-intervento`;\n")
    if additions:
