#!/usr/bin/env python3
"""Materializza la preview v0.4 coverage-first sopra il renderer v0.3."""
...
def _overview(payload: dict[str, Any]) -> str:
    ...
    icons = base.ICONS
    return f'''<section class="method-detail page-width op-overview" aria-label="Quadro operativo"><div class="op-overview-shell"><div class="op-overview-heading"><div><span class="section-number">01</span><h2>Quadro operativo</h2></div><p>La v0.4 separa ciò che è aperto, ciò che resta a sportello e ciò che è già annunciato.</p></div><div class="op-overview-grid op-overview-grid-v04"><article class="op-stat op-stat-open"><span class="op-stat-icon">{icons['briefcase']}</span><div><small>Aperte</small><strong>{opened}</strong><span>Finestra di candidatura attiva</span></div></article><article class="op-stat"><span class="op-stat-icon">{icons['calendar']}</span><div><small>A sportello</small><strong>{rolling}</strong><span>Misure senza singola scadenza</span></div></article><article class="op-stat"><span class="op-stat-icon">{icons['radar']}</span><div><small>In arrivo</small><strong>{upcoming}</strong><span>Procedure annunciate ufficialmente</span></div></article><article class="op-stat op-stat-sources"><span class="op-stat-icon">{icons['radar']}</span><div><small>Fonti monitorate</small><strong>{monitored}</strong><span>{active} attive · {degraded} degradate</span></div></article><article class="op-stat op-stat-towns"><span class="op-stat-icon">{icons['map']}</span><div><small>Famiglie coperte</small><strong>{covered_families}/{required_families}</strong><span>Regionali, nazionali, UE e tematiche</span></div></article><article class="op-stat op-stat-archive"><span class="op-stat-icon">{icons['archive']}</span><div><small>In archivio</small><strong>{len(archive)}</strong><span>Opportunità chiuse con fonte ufficiale</span></div></article></div><div class="op-monitor"><div class="op-monitor-head"><strong>Fonti monitorate</strong><span>Discovery e classificazione sono separati: una segnalazione entra qui solo dopo verifica sulla fonte primaria.</span></div><div class="op-monitor-list">{''.join(base.monitor_markup(x) for x in coverage.get('rows') or [])}</div></div></div></section>'''
...
def render_page(payload: dict[str, Any]) -> str:
    ...
    page = page.replace(
        ...
        'Filtra per Comune, fonte, stato o modalità di partecipazione.',
        1,
    )
    page = page.replace(
        'Bandi e linee di finanziamento con un <strong>ruolo operativo documentato</strong> per almeno un Comune della Versilia.',
        'Bandi, avvisi, incentivi e linee di finanziamento con un <strong>ruolo operativo documentato</strong> per almeno un Comune della Versilia.',
        1,
    )
    return page
...
def build(payload_path: Path, dist: Path) -> Path:
    ...
    if 'Anteprima v0.4' not in text or 'name="robots" content="noindex,nofollow,noarchive"' not in text:
        raise SystemExit("Preview v0.4/noindex non materializzata correttamente")
...
def main() -> int:
    ...
    print(f"Preview opportunità v0.4 materializzata: {build(args.data, args.dist)}")
    return 0
