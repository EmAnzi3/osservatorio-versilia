...
def browser_checks() -> None:
    ...
    with server(DIST) as base, sync_playwright() as p:
        ...
        page.goto(base + "confronta/demografia/?indicatore=populationChange", wait_until="networkidle")
        ...
        page.goto(base + "comuni/massarosa/?tema=demografia&indicatore=populationChange", wait_until="networkidle")
        page.wait_for_selector(".versilia-position")
        ...
        assert page.locator(".all-indicators, .indicator-groups").count() == 0
        assert page.locator('[data-metric="populationChange"]').count() == 1
