...
Apre esclusivamente pagine ufficiali già usate dal Radar. Se favicon/manifest non
sono materializzabili, cattura con Chromium il logo/brand mark realmente
renderizzato nella pagina e lo salva localmente. Nessun placeholder inventato.
"""
...
def _pages_for(source_id: str, opportunities: list[dict[str, Any]]) -> list[str]:
    ...
    return list(dict.fromkeys(url for url in pages if url.startswith("http")))
