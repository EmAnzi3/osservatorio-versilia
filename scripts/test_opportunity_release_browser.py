...
FORBIDDEN = ["Anteprima tecnica","Collaudo","non pubblicata","revisione interna","prototipo","Audit indipendente","capture rate","buchi baseline","fonti di controllo","Famiglie presidiate","Rete di raccolta","Quality gate","coverageHold","Le schede sono ordinate per scadenza","Colore e segno grafico"]
...
def verify_release(base: str) -> None:
    ...
    with sync_playwright() as pw:
        ...
        for width, height in ((1440,1000),(1024,768),(390,844)):
            ...
            root = page.locator('[data-opportunity-preview]'); root.wait_for()
            ...
            assert date_expected in body and 'Radar v0.4.3' in body
            for token in FORBIDDEN: assert token.lower() not in body.lower(), token
            source = page.locator('[data-op-source]')
            assert source.count() == 1 and source.locator('option').count() >= 40
            current = source.locator('option[data-current-count]').evaluate_all("els=>els.map(o=>({value:o.value,count:Number(o.dataset.currentCount||0)})).filter(x=>x.value&&x.count>0)")
            assert current
            source.select_option(current[0]['value']); page.wait_for_timeout(120)
            assert page.locator('[data-opportunity-card]:not([hidden])').count() >= 1
            page.locator('[data-op-reset]').click(); lifecycle=page.locator('[data-op-lifecycle]'); lifecycle.select_option('rolling_open'); page.wait_for_timeout(120)
            visible=page.locator('[data-opportunity-card]:not([hidden])')
            assert visible.count() >= 1 and all(x=='rolling_open' for x in visible.evaluate_all('els=>els.map(e=>e.dataset.lifecycle)'))
            page.locator('[data-op-reset]').click()
            images=page.locator('img[src*="source-favicons/"]'); assert images.count()>=1
            assert not images.evaluate_all('els=>els.filter(i=>!i.complete||i.naturalWidth<1).map(i=>i.src)')
            mic=page.locator('img[src*="source-favicons/mic-dgcc.png"]'); assert mic.count()>=1 and mic.first.evaluate('i=>i.complete&&i.naturalWidth>0')
            assert not page.evaluate('document.documentElement.scrollWidth > document.documentElement.clientWidth + 2')
            context.close()
        browser.close()
    sitemap=(ROOT/'dist/sitemap.xml').read_text(encoding='utf-8')
    assert sitemap.count('https://osservatorioversilia.it/opportunita/') == 1
    print(f'Radar pubblico browser OK: {total_expected} opportunità · {date_expected} · header/home/footer · 1440/1024/390.')
