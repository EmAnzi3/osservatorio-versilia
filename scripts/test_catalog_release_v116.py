...
def main() -> None:
    ...
    assert 'UX_ASSET_VERSION = "20260830-v125-erp-arrears"' in build_safe
    assert 'HISTORY_ASSET_VERSION = "20260830-v125-erp-arrears"' in build_safe
    assert 'APP_BUNDLE_ASSET_VERSION = "20260830-v125-erp-arrears"' in build_brand
    assert 'CHART_SURFACE_ASSET_VERSION = "20260826-v120"' in build_brand
    assert 'PWA_JS_REVISION = "catalog-v125"' in build_brand
    assert "20260830-v125-erp-arrears" in development_loader
    assert "const VERSION = '20260830-v125-erp-arrears';" in export_script
    assert "function sexBreakdownRows(" in export_script
    assert "ov-pwa-20260830-v125-erp-arrears" in service_worker
