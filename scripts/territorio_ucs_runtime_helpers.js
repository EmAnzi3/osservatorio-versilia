  ...
  function territorialClassificationTownMarkup(metric,row) {
    ...
    </div><p class="aggregate-note territorial-classification-note">Litoraneità e zona costiera sono classificazioni distinte: un Comune può non confinare con il mare e ricadere comunque nella zona costiera Istat.</p>`;
  }
  ...
  function territorialClassificationCompareMarkup(data,metricKey) {
    ...
    return `<div class="territorial-classification-shell">
      ...
      <p class="composite-compare-note">Le classi DEGURBA sono ordinali e non vengono mediate. Il riepilogo Versilia usa esclusivamente conteggi di Comuni per classe.</p>
    </div>`;
  }
  ...
  function landCoverChartMarkup(metric,source,categoryKey='artificialized',unit='percent',label='Territorio') {
    ...
    const max=unit==='percent'?100:Math.max(...values.map(value=>Number(value)||0),1);
    const chart=values.length===years.length?`<div class="land-cover-trend" role="img" aria-label="${html(def.label||metric.meta.label)} · ${html(label)} · serie ${html(metric.meta.year)}"><div class="land-cover-trend-head"><span>Anno</span><span>Scala da zero</span><span>Valore</span></div>${years.map((year,index)=>{const value=Number(values[index])||0;return `<div class="land-cover-trend-row"><strong>${year}</strong><span class="land-cover-trend-track"><i style="width:${Math.max(value>0?1:0,value/max*100)}%"></i></span><b>${html(landCoverFormat(value,unit))}</b></div>`;}).join('')}</div>`:'<p>Serie non disponibile.</p>';
  ...
  function landCoverCompareRows(data,metricKey,category,year,unit) {
    ...
    const max=unit==='percent'?100:Math.max(...rows.map(row=>Math.abs(Number(row.displayValue)||0)),.0001);
    return rows.map((row,rank)=>{const q=new URLSearchParams({tema:metric.meta.theme,indicatore:metricKey}),formatted=landCoverFormat(row.displayValue,unit);return `<a class="bar-row" href="${route(`comuni/${row.slug}/?${q}`)}" aria-label="${html(row.town)}: ${html(formatted)}"><span class="bar-rank">${rank+1}</span><span class="bar-town">${html(row.town)}</span><span class="bar-track"><span class="bar-fill" style="width:${Math.max(1.5,Math.abs(Number(row.displayValue)||0)/max*100)}%"></span><span class="bar-hover-label">${html(row.town)} · ${html(formatted)}</span></span><strong>${html(formatted)}</strong></a>`;}).join('');
  }
  ...
  function landCoverTransformationCompareMarkup(metric) {
    ...
    const max=Math.max(...rows.map(row=>Number(row.transformation?.changedPct)||0),.0001);
    return `<div class="land-cover-transform-compare"><div class="land-cover-subheading"><span class="overline">Trasformazione 2007→2019</span><h4>Superficie che cambia macroclasse UCS</h4></div><div class="comparison-bars">${rows.map((row,index)=>{const t=row.transformation||{},q=new URLSearchParams({tema:metric.meta.theme,indicatore:metric.meta.key});return `<a class="bar-row" href="${route(`comuni/${row.slug}/?${q}`)}"><span class="bar-rank">${index+1}</span><span class="bar-town">${html(row.town)}</span><span class="bar-track"><span class="bar-fill" style="width:${Math.max(1.5,(Number(t.changedPct)||0)/max*100)}%"></span></span><strong>${html(landCoverFormat(t.changedPct,'percent'))}</strong></a>`;}).join('')}</div><p class="aggregate-note">Versilia: <b>${html(landCoverFormat(metric.aggregate?.transformation?.changedPct,'percent'))}</b> (${html(landCoverFormat(metric.aggregate?.transformation?.changedHa,'hectares'))}). Il valore Versilia è Σ ha cambiati / Σ ha UCS, non la media delle percentuali comunali.</p></div>`;
  }
  ...
  function landCoverCompareMarkup(data,metricKey,view={}) {
    ...
    return `<div class="land-cover-shell land-cover-compare-shell" data-land-cover-compare-shell>
      ...
      <div class="comparison-bars">${landCoverCompareRows(data,metricKey,category,year,unit)}</div>
